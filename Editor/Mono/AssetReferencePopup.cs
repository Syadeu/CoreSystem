...
namespace SyadeuEditor
{
    ...
    public sealed class SelectorPopup<T, TA> : PopupWindowContent
    {
        ...
        private SelectorPopup(Action<T> setter, Func<TA, T> getter, List<TA> list)
        {
            m_SearchField = new SearchField();
            m_Setter = setter;
            m_Getter = getter;
        ...
        public static SelectorPopup<T, TA> GetWindow(List<TA> list, Action<T> setter, Func<TA, T> getter)
        {
            if (m_Window == null)
            {
                m_Window = new SelectorPopup<T, TA>(setter, getter, list);
            }
            else
            {
                m_Window.m_Setter = setter;
        ...
        private class SelectorTreeView : TreeView
        {
            ...
            protected override TreeViewItem BuildRoot()
            {
                ...
                {
                    root.AddChild(new AttributeTreeViewItem(i + 1, 0, list[i].ToString(), m_Popup.m_Getter.Invoke(list[i])));
                }
                return root;
            }
        }
